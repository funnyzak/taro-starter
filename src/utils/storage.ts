...
export const currentContent = (): Promise<IContentResponse> => {
    const params: IPageParams = (getCurrentInstance().router as RouterInfo).params;
    const _contentId = params.id && params.id !== null ? params.id : contentId;

    const requestContent = (id: number, resolve, defContent?: IContentResponse) => {
        Taro.showLoading({
            title: '数据加载中..',
        })
        api.Content.get(id).then(res => {
    ...
    return new Promise<IContentResponse>((resolve) => {
        ...
        Taro.getStorage({
            ...
            success: res => {
                rltContent = res.data
                if ((_contentId > 0 && _contentId !== requested_content_id && _contentId !== rltContent.id)) {
