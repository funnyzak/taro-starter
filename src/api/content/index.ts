...
function get(id: number, chnId: number = channelId) {
  return request.get(GET_CONTENT.replace('$id', id.toString()).replace('$channel', chnId.toString()))
}

function digg(id: number, chnId: number = channelId) {
  return httpRequest({ 
    url:DIGG_CONTENTS.replace('$id', id.toString()).replace('$channel', chnId.toString()), 
    data:{}, 
    headers: [
